...
export async function deployVercel(config) {
    ...
    try {
        ...
        // Deploy
        const { stdout } = await execa("vercel", ["deploy", "--prod", "--yes"], {
            cwd: frontendDir,
            env: { VERCEL_TOKEN: config.vercelToken },
        });
...
async function deployVercelApi(config) {
    // Create project
    ...
    if (config.railwayUrl) {
        await fetch(`${VERCEL_API}/v10/projects/${config.projectName}/env`, {
            method: "POST",
            headers: {
                Authorization: `Bearer ${config.vercelToken}`,
                "Content-Type": "application/json",
...
async function updateRailwayFrontendUrl(config) {
