...
async function main() {
  ...
  if (!config.skipDeploy) {
    ...
    config.githubUsername = githubUsername as string;
    ...
    config.githubToken = githubToken as string;
    ...
    config.railwayToken = railwayToken as string;
    ...
    config.vercelToken = vercelToken as string;
  }
  ...
  try {
    ...
    if (!config.skipGitHub) {
      console.log(
        `    Repo:     ${chalk.cyan(`https://github.com/${config.githubUsername}/${config.projectName}`)}`
      );
    }
    console.log();
