...
export async function setupGitHub(config) {
    ...
    await addRepoSecret(config.githubUsername, config.projectName, config.githubToken, "RAILWAY_TOKEN", config.railwayToken);
    await addRepoSecret(config.githubUsername, config.projectName, config.githubToken, "VERCEL_TOKEN", config.vercelToken);
}
async function addRepoSecret(owner, repo, token, secretName, secretValue) {
    // Get repo public key for encryption
    const keyResponse = await fetch(`${GITHUB_API}/repos/${owner}/${repo}/actions/secrets/public-key`, {
        headers: {
    ...
    const secretResponse = await fetch(`${GITHUB_API}/repos/${owner}/${repo}/actions/secrets/${secretName}`, {
        method: "PUT",
        headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
...
async function encryptSecret(publicKey, secretValue) {
    ...
    const encrypted = sodium.box(messageBytes, nonce, keyBytes, ephemeralKeyPair.secretKey);
