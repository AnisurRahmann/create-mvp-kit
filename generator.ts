...
export async function createProject(config: Config): Promise<void> {
...
async function generateBackend(projectDir: string, config: Config) {
  const backendDir = path.join(projectDir, "backend");
  await fs.ensureDir(backendDir);

  // api.py
  await fs.writeFile(
    path.join(backendDir, "api.py"),
...
async function generateFrontend(projectDir: string, config: Config) {
...
async function generateGitHubActions(projectDir: string, config: Config) {
...
async function generateRootFiles(projectDir: string, config: Config) {
